...
const PlateList = ({ plates, messages }) => {
  ...
  const userId = localStorage.getItem('userId') || 'guest'; // Get current user ID
  ...
  const handleReport = (senderId) => {
    if (!senderId) return;
    if (reportedSenders.includes(senderId)) return;
        ...
        <li key={p._id}>
          ...
          <ul>
            {messages
              ...
              .map((m) => (
                <li key={m._id} style={{ fontSize: '0.9rem', color: '#555', marginBottom: '0.5rem' }}>
                  {m.message}{' '}
